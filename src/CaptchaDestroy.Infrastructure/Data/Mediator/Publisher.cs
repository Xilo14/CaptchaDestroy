...
namespace CaptchaDestroy.Infrastructure.Data.Mediator
{
    ...
    {
        private readonly ServiceFactoryNested _serviceFactoryNested;
        private readonly ServiceFactoryNeighboured _serviceFactoryNeighboured;

        public Publisher(
            ServiceFactoryNeighboured serviceFactoryNeighboured,
            ServiceFactoryNested serviceFactoryNested)
        {
            _serviceFactoryNested = serviceFactoryNested;
            _serviceFactoryNeighboured = serviceFactoryNeighboured;

            PublishStrategies[PublishStrategy.Async] = new CustomMediator(_serviceFactoryNested, AsyncContinueOnException);
            PublishStrategies[PublishStrategy.ParallelNoWait] = new CustomMediator(_serviceFactoryNeighboured, ParallelNoWait);
            PublishStrategies[PublishStrategy.ParallelWhenAll] = new CustomMediator(_serviceFactoryNested, ParallelWhenAll);
            PublishStrategies[PublishStrategy.ParallelWhenAny] = new CustomMediator(_serviceFactoryNeighboured, ParallelWhenAny);
            PublishStrategies[PublishStrategy.SyncContinueOnException] = new CustomMediator(_serviceFactoryNested, SyncContinueOnException);
            PublishStrategies[PublishStrategy.SyncStopOnException] = new CustomMediator(_serviceFactoryNested, SyncStopOnException);
        }
        ...
        {
            if (notification is IHasPublishStrategy notificationWithPublishStrategy
                && notificationWithPublishStrategy.PublishStrategy != null)
                return Publish(notification,
                 (notification as IHasPublishStrategy).PublishStrategy.Value, default(CancellationToken));
